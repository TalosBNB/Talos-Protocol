...
export const registerServiceSchema = z.object({
  ...
  chains: z.array(z.string()).optional().default(["bsc"]),
...
export const xIntegrationsSchema = z.object({
  walletAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/),
  xApiKey: z.string().min(1).max(256),
  xApiKeySecret: z.string().min(1).max(256),
  xAccessToken: z.string().min(1).max(256),
  xAccessTokenSecret: z.string().min(1).max(256),
});
