                ...
                {revenueStreams.map((rs) => (
                  <div key={rs.talosId} className="bg-surface border border-border p-6">
                    ...
                    <div className="mb-4 px-3 py-2 bg-background border border-border text-xs text-muted">
                      100% Agent Treasury &mdash; USDC service revenue to agent wallet
                    </div>
                ...
                <div key={c.id} className="bg-surface border border-border p-5">
                  ...
                  <div className="space-y-2 text-xs">
                    {c.hasFlapToken ? (
                      ...
                      <div className="text-muted py-1">No community token — service-only Talos</div>
                    )}
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-foreground">Agent Treasury (USDC)</span>
                    </div>
                    {c.agentWalletAddress && (
                      <a
                        href={`https://bscscan.com/address/${c.agentWalletAddress}`}
                        target="_blank"
                        rel="noopener noreferrer"
            ...
            <div className="bg-surface border border-border divide-y divide-border">
              {talosManagement.map((c) => (
                <div key={c.id} className="p-5 hover:bg-surface-hover transition-colors">
                  ...
                  {c.channels.some((ch) => ch.toLowerCase().includes("x") || ch.toLowerCase().includes("twitter")) && (
                    <XApiCredentialsForm
                      talosId={c.id}
                      integration={c.xIntegration}
