...
function LaunchForm() {
  ...
  const [genesisResult, setGenesisResult] = useState<{
    ...
    agentName: string;
    memeUrl: string | null;
    memeLaunched: boolean;
    memeSkipReason: string | null;
  ...
  const canNext = () => {
    switch (step) {
      ...
      case 1:
        return true;
      case 2: {
        if (!form.enableFlapToken) return true;
  ...
  const handleLaunch = async () => {
    ...
    try {
      // 1. Verify EVM wallet + ensure BNB Smart Chain (56)
      ...
      if (TALOS_REGISTRY_CONTRACT) {
        setDeployStep("Registering TALOS on BSC...");
        setDeployProgress(2);
      ...
      let dbData: {
        id: string;
        apiKeyOnce: string;
        agentWalletKeyOnce: string | null;
        agentWalletAddress?: string | null;
        memeUrl?: string | null;
        memeLaunched?: boolean;
        memeSkipReason?: string | null;
        flapUrl?: string | null;
        flapLaunched?: boolean;
        flapSkipReason?: string | null;
      } | null = null;
      try {
        const res = await fetch("/api/talos", {
          ...
          body: JSON.stringify({
            ...
            launchFlapToken: form.enableFlapToken,
            tokenSymbol: form.enableFlapToken ? form.tokenSymbol : undefined,
      ...
      setGenesisResult({
        ...
        onChainId,
        agentName: form.agentName,
        memeUrl: dbData!.memeUrl ?? dbData!.flapUrl ?? null,
        memeLaunched: !!(dbData!.memeLaunched ?? dbData!.flapLaunched),
        memeSkipReason: dbData!.memeSkipReason ?? dbData!.flapSkipReason ?? null,
        agentWalletAddress: dbData!.agentWalletAddress ?? null,
      });
    } catch (err) {
      ...
      if (raw.includes("User declined") || raw.includes("user rejected") || raw.includes("User rejected"))
        ...
        message = "Insufficient BNB for gas.";
    ...
    return (
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="bg-surface border border-accent/30 p-8 mb-6 space-y-6">
          ...
          {genesisResult.agentWalletAddress && (
            <div>
              <div className="text-xs text-muted mb-2">Agent Wallet (receives USDC service revenue)</div>
              <div className="text-foreground font-mono text-xs break-all">{genesisResult.agentWalletAddress}</div>
            </div>
          )}
          ...
          {genesisResult.memeLaunched && genesisResult.memeUrl ? (
            <div>
              ...
              <a
                ...
                className="text-sm text-accent hover:text-foreground underline break-all"
              >
                {genesisResult.memeUrl}
              </a>
            </div>
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">This EVM address will be registered as the TALOS Creator on BSC.</p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <label className="flex items-start gap-3 p-4 border border-border bg-background cursor-pointer">
              <input
                type="checkbox"
                checked={form.enableFlapToken}
                onChange={(e) => setForm((prev) => ({ ...prev, enableFlapToken: e.target.checked }))}
                className="mt-1"
              />
            ...
            {form.enableFlapToken ? (
              ...
              <div className="p-4 border border-dashed border-border text-xs text-muted">
                Skipping token. Your agent can still sell services and earn USDC via x402.
              </div>
            )}
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-sm">
              ...
              />
              {form.enableFlapToken && (
                <ReviewRow label="Reference supply" value={Number(form.totalSupply).toLocaleString()} />
              )}
