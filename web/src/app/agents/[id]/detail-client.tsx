...
const USDC_DECIMALS = Number(process.env.NEXT_PUBLIC_USDC_DECIMALS ?? 18);
...
export function TalosDetailClient({ talos }: { talos: TalosDetail }) {
  ...
  const syncPatronFromChain = useCallback(async () => {
    ...
    const res = await fetch(`/api/talos/${talos.id}/buy-token`, {
      ...
      body: JSON.stringify({ buyerPublicKey: address }),
    });
    const data = await res.json();
    if (res.ok && typeof data.onChainBalance === "number") {
      setPulseBalance(data.onChainBalance);
  ...
  const handleBecomePatron = useCallback(async () => {
    ...
    try {
      const { ok, data } = (await syncPatronFromChain()) ?? { ok: false, data: null };
      if (ok && data?.success) {
        if (data.patronStatus === "registered" || data.patronStatus === "updated") {
          setPatronStatus("patron");
        } else {
        ...
        alert(data?.error || "Failed to sync patron status");
        setPatronStatus("none");
      }
    } catch {
  ...
  }, [address, isPatron, syncPatronFromChain, minRequired, talos.tokenSymbol]);

  // ─── Buy Token modal state ──────────────────────────
  const [buyOpen, setBuyOpen] = useState(false);
  const [buyStatus, setBuyStatus] = useState<"idle" | "syncing" | "success" | "error">("idle");
  const [buyResult, setBuyResult] = useState<{ message: string; balance?: number } | null>(null);
  ...
  const handleRequestService = useCallback(async () => {
    ...
    try {
      // Pay USDC on BSC to the service recipient (agent wallet or operator).
      const recipient =
      ...
      alert(msg.toLowerCase().includes("insufficient") ? "Insufficient BNB for gas." : msg);
      setServiceStatus("error");
    }
  ...
  const handleSyncAfterBuy = useCallback(async () => {
    if (!address) return;
    setBuyStatus("syncing");
    try {
      const { ok, data } = (await syncPatronFromChain()) ?? { ok: false, data: null };
      if (ok && data?.success) {
      ...
      console.error("[buy-token sync]", err);
      alert(err instanceof Error ? err.message : "Sync failed");
      setBuyStatus("error");
    }
  }, [address, syncPatronFromChain]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-12">
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 gap-px bg-border mb-8">
        {[
          { label: `${talos.tokenSymbol} Price`, value: priceDisplay },
            ...
            <div className="bg-surface border border-border p-6">
              ...
              <div className="space-y-3 text-xs">
                ...
                {talos.hasFlapToken ? (
                  ...
                  <div className="text-muted">No community token — USDC services only</div>
                )}
                {talos.agentWalletAddress && (
                  <div className="flex justify-between">
            ...
            <>
              ...
              <div className="bg-surface border border-border p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-foreground mt-1">x402 on BSC</p>
                  </div>
                  <div>
              ...
              <div className="bg-surface border border-border p-6">
                ...
                <div className="bg-background border border-border p-4 text-xs text-foreground overflow-x-auto font-mono space-y-1">
                  <div className="text-green-400"># 1. Send USDC payment on BSC</div>
                  ...
                  <div className="pl-4">&quot;txHash&quot;: &quot;&lt;bsc_tx_hash&gt;&quot;,</div>
      ...
      {tab === "Agent" && (
        <div className="space-y-6">
          ...
          <div className="bg-surface border border-border p-6">
            ...
            {talos.channels.some((ch) => ch.toLowerCase().includes("x") || ch.toLowerCase().includes("twitter")) && (
              <XApiCredentialsForm talosId={talos.id} integration={talos.xIntegration} />
            )}
          </div>
          ...
          <div className="bg-surface border border-border p-6">
            ...
            <div className="bg-background border border-border p-4 text-xs text-foreground space-y-1 overflow-x-auto font-mono">
              ...
              <div className="text-muted">Network:   BNB Smart Chain</div>
          ...
          <div className="relative bg-background border border-border w-full max-w-md mx-4 p-0">
            ...
            {buyStatus === "success" && buyResult ? (
              ...
              <div className="px-6 py-8 text-center">
                ...
                <p className="text-sm text-foreground mb-2">Balance Synced</p>
                <p className="text-xs text-muted mb-4">{buyResult.message}</p>
                {buyResult.balance != null && (
                  <div className="bg-surface border border-border p-3 text-xs font-mono text-muted mb-6">
                    {buyResult.balance.toLocaleString()} {talos.tokenSymbol} held
                  </div>
                )}
                <button
              ...
              <div className="px-6 py-6 space-y-5">
                ...
                <div className="bg-surface border border-border p-4 space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span className="text-muted">Patron threshold</span>
                    <span className="text-foreground">{minRequired.toLocaleString()} {talos.tokenSymbol}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted">Your balance (synced)</span>
                    <span className="text-foreground">{myPulseBalance.toLocaleString()} {talos.tokenSymbol}</span>
                  </div>
                </div>
                ...
                <button
                  onClick={handleSyncAfterBuy}
                  disabled={!address || buyStatus === "syncing" || !memeUrl}
                  className="w-full py-3 text-sm font-medium border border-accent/40 text-accent hover:bg-accent hover:text-background transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {buyStatus === "syncing" ? "Syncing..." : "Sync wallet balance"}
                </button>
              ...
              <div className="px-6 py-6 space-y-5">
                ...
                <div className="bg-surface border border-border p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-foreground">BNB Smart Chain · wallet signing</span>
                  </div>
                </div>
