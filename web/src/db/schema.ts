...
export const tlsTalos = pgTable(
  "tls_talos",
  {
    ...
    onChainId: integer("onChainId").unique(),        // BNB Smart Chain TalosRegistry on-chain ID
    ...
    // Mitos Token (per-agent ERC-20 on BSC)
...
export const tlsCommerceServices = pgTable(
  "tls_commerce_services",
  {
    ...
    chains: text("chains").array().notNull().default(["bsc"]),
...
export const tlsCommerceJobs = pgTable(
  "tls_commerce_jobs",
  {
    ...
    txHash: text("txHash"),                    // BNB Smart Chain transaction hash after settlement
    amount: numeric("amount", { precision: 18, scale: 6 }).notNull(),
