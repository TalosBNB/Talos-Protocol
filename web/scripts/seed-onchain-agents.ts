/**
 * On-chain seed: registers example Talos agents on BNB Smart Chain and writes
 * the full off-chain projection to Supabase so the Agents / Activity / Leaderboard
 ...
 * the global Activity feed shows verifiable BscScan hashes.
 *
...
const OPERATOR_KEY = process.env.BSC_OPERATOR_PRIVATE_KEY as Hex;
const OPERATOR_ADDRESS = process.env.BSC_OPERATOR_ADDRESS as Address;
...
if (!OPERATOR_KEY || !OPERATOR_ADDRESS) throw new Error("BSC_OPERATOR_PRIVATE_KEY / _ADDRESS missing");
...
// ── Main ──────────────────────────────────────────────────────────────
async function main() {
  console.log(`\n🌱 On-chain seed → BNB Smart Chain (chain ${CHAIN_ID})`);
  ...
  for (const def of AGENTS) {
    ...
    const acts = [
      { type: "genesis", channel: "System", content: `${def.name} achieved Genesis on BSC — onChainId ${onChainId}, ${def.agentName}.talos registered.`, at: daysAgo(5, 2) },
