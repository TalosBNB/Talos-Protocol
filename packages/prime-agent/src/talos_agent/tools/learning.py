...
async def run_performance_review() -> dict:
    ...
    profile = _settings.llm_profile
    if not profile:
        return {"status": "analysis_failed", "error": "No LLM configured"}
...
async def evolve_strategy() -> dict:
    ...
    profile = _settings.llm_profile
    if not profile:
        return {"status": "failed", "error": "No LLM configured"}
    client = get_llm_client(profile)
    response = await client.chat(
        messages=[
            {
    ...
    try:
        playbook_data = json.loads(response.content or "{}")
    except (json.JSONDecodeError, IndexError):
