"""EVM / BNB Smart Chain tools — internal economy (Pulse tokens, BNB dividends, governance).

All operations are proxied through the Talos Web API or read from the BNB Smart Chain
...
@tool("transfer_bnb", "Transfer native BNB to a BNB Smart Chain (0x) address (dividends, payments). Auto-checks approval threshold.")
async def transfer_bnb(to_account: str, amount: float, reason: str = "") -> dict:
    kit = _get_kit()
    ...
    if amount >= threshold:
        result = await _api.create_approval(
            _settings.talos_id,
            type_="transaction",
            title=f"BNB transfer: {amount} to {to_account}",
            description=reason,
            amount=amount,
        )
        return {
...
@tool("get_bnb_balance", "Check native BNB balance for the Talos address via BNB Smart Chain RPC (eth_getBalance)")
async def get_bnb_balance() -> dict:
    kit = _get_kit()
    await kit.initialize()
...
async def create_pulse_token(name: str, symbol: str, initial_supply: int = 1000000) -> dict:
    result = await _api.create_approval(
        ...
        description=f"Initial supply: {initial_supply}. Token deployment handled by Web on BSC (EVM).",
    )
    return {
        "status": "approval_requested",
        "approval_id": result.get("id") if result else None,
...
@tool("execute_approved_transfer", "Execute a previously approved BNB or token transfer. Call after check_approval returns 'approved'.")
async def execute_approved_transfer(to_account: str, amount: float, currency: str = "BNB", token_id: str = "") -> dict:
