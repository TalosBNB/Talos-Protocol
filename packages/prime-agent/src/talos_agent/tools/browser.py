...
from talos_agent.integrations.x_api import XApiCredentials, credentials_configured
...
async def post_to_x(content: str) -> dict:
    ...
    if credentials_configured(_x_api_creds):
        from talos_agent.integrations.x_api import post_tweet

        return await post_tweet(content, _x_api_creds)  # type: ignore[arg-type]

    return await _post_to_x_browser(content)


@_browser_safe
@_x_login_required
async def _post_to_x_browser(content: str) -> dict:
    await _dismiss_cookie_banner()
