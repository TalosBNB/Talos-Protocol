...
async def run(settings: Settings, agent_slot: int = 0) -> None:
    ...
    integrations_raw = await api.get_integrations()
    from talos_agent.integrations import parse_integrations

    integrations = parse_integrations(integrations_raw)
    if integrations.x:
        console.print("[green]X API credentials loaded from Talos Web.[/green]")
    elif "X" in str(talos_config.get("channels", [])) or "Twitter" in str(talos_config.get("channels", [])):
        console.print("[yellow]X channel enabled but API keys not set — add them in Dashboard → TALOS Management.[/yellow]")
