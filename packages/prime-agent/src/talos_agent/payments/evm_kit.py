"""EVM / BNB Smart Chain operations — delegates to Web API and reads chain via JSON-RPC.

The Prime Agent never holds private keys. Read operations (native BNB
balance) query the BNB Smart Chain RPC directly via ``eth_getBalance``. Write
...
# BNB Smart Chain RPC (native gas token: BNB)
_RPC_URL = os.getenv("BSC_RPC_URL", "https://bsc-dataseed.binance.org")

# BNB native token uses 18 decimals (EVM standard for native gas).
_WEI = 10**18
...
class EvmKit:
    """Proxy for BNB Smart Chain (EVM) operations via Talos Web API + JSON-RPC.

    Read operations use the public BNB Smart Chain RPC (no key needed).
    ...
    async def initialize(self) -> None:
        ...
        console.print("[green]BNB Smart Chain proxy ready (via Web API + JSON-RPC).[/green]")
    ...
    async def _rpc_call(self, method: str, params: list[Any]) -> Any:
        """Make a JSON-RPC call to the BNB Smart Chain node."""
    ...
    async def get_balance(self, account_id: str = "") -> dict[str, Any]:
        """Query native BNB balance via eth_getBalance (public RPC)."""
        try:
            ...
            wei = int(result, 16)
            return {"balance_bnb": wei / _WEI, "wei": wei, "account": acct}
    ...
    async def transfer_native(self, to_account: str, amount: float) -> dict[str, Any]:
        """Request a native BNB transfer via Web API (Web handles signing)."""
        try:
            result = await self._api.request_transfer(
                to_account=to_account, amount=amount, currency="BNB"
            )
