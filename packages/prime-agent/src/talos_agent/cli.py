...
def start(talos_id: str | None, env_file: str):
    ...
    if not all_keys:
        ...
        sys.exit(1)
    if not settings.llm_profile:
        console.print(
            "[red]Error:[/red] No LLM configured. Set one provider key, e.g.\n"
            "  GROQ_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY, KIMI_API_KEY, DEEPSEEK_API_KEY, GOOGLE_API_KEY\n"
            "  or LLM_API_KEY + LLM_BASE_URL + LLM_MODEL for any OpenAI-compatible API."
        )
        sys.exit(1)
    ...
    console.print(f"  LLM:       {profile.provider} / {profile.model}")
    console.print()
...
@click.option("--llm-key", prompt="LLM API Key", default=None, help="Groq, OpenAI, Anthropic, Kimi, DeepSeek, Google, …")
@click.option("--llm-provider", default=None, help="Provider preset: groq, openai, anthropic, kimi, deepseek, google")
def config(api_key: str, llm_key: str | None, llm_provider: str | None):
    ...
    updates: dict[str, str] = {"talos_api_key": api_key}
    if llm_key:
        if llm_provider == "anthropic":
            updates["anthropic_api_key"] = llm_key
        elif llm_provider in ("kimi", "moonshot"):
            updates["kimi_api_key"] = llm_key
        elif llm_provider == "deepseek":
            updates["deepseek_api_key"] = llm_key
        elif llm_provider == "google":
            updates["google_api_key"] = llm_key
        elif llm_provider == "groq":
            updates["groq_api_key"] = llm_key
        else:
            updates["openai_api_key"] = llm_key
    if llm_provider:
