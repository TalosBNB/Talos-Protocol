...
class TalosAPIClient:
    ...
    async def get_integrations(self) -> dict | None:
        """Fetch X and other channel credentials stored in Talos Web."""
        if not self._talos_id:
            return None
        r = await self._client.get(f"/api/talos/{self._talos_id}/integrations")
    ...
    async def create_agent_wallet(self) -> dict | None:
        """Create an EVM/BNB Smart Chain wallet for this Talos if one doesn't exist."""
    ...
    async def purchase(
        ...
        *,
        ...
        settle on BSC) and returns ``{status, httpStatus, data, receipt}``
