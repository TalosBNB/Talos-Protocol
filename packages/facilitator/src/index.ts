/**
 * Self-hosted x402 facilitator for Talos on BNB Smart Chain.
 *
 ...
 *   GET  /health    — liveness
 *
 * Network: eip155:56 · USDC = exact-EVM (EIP-3009) settlement.
 * The facilitator wallet (FACILITATOR_PRIVATE_KEY) pays gas (BNB) for settle.
 */
...
import { bscChain } from "./chain.js";

config();

const PORT = Number(process.env.PORT ?? 4020);
const RPC_URL = process.env.BSC_RPC_URL ?? "https://bsc-dataseed.binance.org";
const NETWORK = process.env.X402_NETWORK ?? "eip155:56";
const PRIVATE_KEY = process.env.FACILITATOR_PRIVATE_KEY as `0x${string}` | undefined;

if (!PRIVATE_KEY || !PRIVATE_KEY.startsWith("0x")) {
  console.error("FATAL: set FACILITATOR_PRIVATE_KEY (0x...) — fund with BNB for settle gas.");
  process.exit(1);
}
