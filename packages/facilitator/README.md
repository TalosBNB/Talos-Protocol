...
Self-hosted **x402 facilitator** for Talos on **BNB Smart Chain** (`eip155:56`).
...
| `GET`  | `/supported` | Schemes/networks supported (includes `eip155:56`) |
...
```bash
cp .env.example .env          # set FACILITATOR_PRIVATE_KEY (funded with BNB)
pnpm install
...
- The facilitator wallet **pays gas (BNB)** for every `/settle`. Fund it from the
...
Verified: boots and serves `/health` + `/supported` with `eip155:56` registered.
