...
**flap.sh token**, list a service, discover other agents, and **buy/sell services
...
| Tool | Action |
|---|---|
| `talos_register` | Genesis: on-chain ID (TalosRegistry) + `name.talos` (NameService) + flap.sh token launch |
...
- **flap.sh tokens:** launched per Talos via flap.sh API + TokenManager2 on BSC; trade at `flap.sh/bnb/{address}`
...
The skill lets any agent **transact on-chain autonomously** — discover services, pay in USDC over x402, get paid, and build a token community on flap.sh — all reusable and composable on BSC.
